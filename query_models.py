...
#             np.save(os.path.join("saved_init_max_usw_soln", dset_name), self.curr_alloc)
#
#     def check_expected_values(self, queries, reviewer, adj_matrix):
#         expected_expected_values = []
#         mqv = 0.0
#         for q in queries:
#             if q in np.where(self.curr_alloc[reviewer, :])[0].tolist():
#                 updated_expected_value_if_no = self._update_alloc_sim(reviewer, q, 0, adj_matrix)
#             else:
#                 updated_expected_value_if_no = self.curr_expected_value
#
#             improvement_ub = self.v_tilde[reviewer, q] * (1 - self.v_tilde[reviewer, q]) + self.curr_expected_value
#
#             if improvement_ub < mqv or math.isclose(improvement_ub, mqv):
#                 expected_expected_values.append(self.curr_expected_value)
#             else:
#                 updated_expected_value_if_yes = self._update_alloc_sim(reviewer, q, 1, adj_matrix)
#
#                 expected_expected_value = self.v_tilde[reviewer, q] * updated_expected_value_if_yes + \
#                                           (1 - self.v_tilde[reviewer, q]) * updated_expected_value_if_no
#                 expected_expected_values.append(expected_expected_value)
#                 if expected_expected_value > mqv:
#                     mqv = expected_expected_value
#         return expected_expected_values
#
#     def get_query(self, reviewer):
#         papers_to_check = sorted(list(set(range(self.n)) - self.already_queried[reviewer]), key=lambda x: random.random())
#
...
#                     adj_matrix[reviewer][paper + self.m] = -self.v_tilde[reviewer, paper]
#
#         expected_expected_values = self.check_expected_values(papers_to_check, reviewer, adj_matrix)
#
#         indices = np.argsort(expected_expected_values)[::-1].tolist()
#         return [papers_to_check[i] for i in indices]
#
...
#         return updated_expected_value, updated_alloc
#
#     def _update_alloc_sim(self, r, query, response, adj_matrix):
#         # We know that if the queried paper is not currently assigned, and its value is 0, the allocation won't change.
#         # print("check value if paper %d for rev %d is %d" % (query, r, response))
#
#         if self.curr_alloc[r, query] < .1 and response == 0:
#             return self.curr_expected_value
#
#         touched_nodes = set()
#
#         # Otherwise, we need to repeatedly check for augmenting paths in the residual graph
...
#             touched_nodes.add(query+self.m)
#         else:
#             # update the weight of the edge from r to query (should be negative).
...
#         #         # It's a rev
#         #         adj_matrix[node, :] = np.inf
#         #         num_papers = np.sum(self.curr_alloc[node, :])
#         #         if num_papers > 0.1:
#         #             adj_matrix[node][self.n + self.m] = 0
#         #         elif adj_matrix[node][self.n+self.m] < np.inf:
#         #             adj_matrix[node][self.n + self.m] = np.inf
#         #         if num_papers < self.loads[node] - .1:
#         #             adj_matrix[self.n + self.m][node] = 0
#         #         elif adj_matrix[self.n + self.m][node] < np.inf:
#         #             adj_matrix[self.n + self.m][node] = np.inf
#         #
#         #         for paper in range(self.n):
#         #             if self.curr_alloc[node, paper] > .5:
#         #                 adj_matrix[paper + self.m][node] = self.v_tilde[node, paper]
#         #                 if adj_matrix[node][paper+self.m] < np.inf:
#         #                     adj_matrix[node][paper+self.m] = np.inf
#         #             else:
#         #                 adj_matrix[node][paper + self.m] = -self.v_tilde[node, paper]
#         #                 if adj_matrix[paper+self.m][node] < np.inf:
#         #                     adj_matrix[paper+self.m][node] = np.inf
#         adj_matrix = adj_matrix_copy
#
#         # assert np.allclose(adj_matrix, adj_matrix_copy)
#
#         return updated_expected_value
